...
void Player::GiveLevel(uint32 level)
{
    ...
    // 15, 30, 40, 50 and 60
    if (level == 15 || (level >= 30 && level <= 60 && level % 10 == 0))
        sWorld.SendWorldText(GetTeam() == ALLIANCE ? LANG_ANNOUNCE_LEVEL_A : LANG_ANNOUNCE_LEVEL_H, ChatHandler(this).GetNameLink(this).c_str(), level);
}
